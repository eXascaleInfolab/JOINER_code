...
public:
    ...
    double calc_sum(int e1,int e2,int rel)
    {
        ...
        if (L1_flag)
            ...
            sum+=- fabs(entity_vec[e1][ii]+relation_vec[rel][ii]-entity_vec[e2][ii]);
        else
        for (int ii=0; ii<n; ii++)
            sum+=-sqr(entity_vec[e1][ii]+relation_vec[rel][ii]-entity_vec[e2][ii]);
        return sum;
    }
    void run(string bu_path)
    {
        FILE* f1 = fopen((bu_path+"relation2vec."+version).c_str(),"r");
        FILE* f3 = fopen((bu_path+"entity2vec."+version).c_str(),"r");
		...
		cout<<bu_path<<"left:"<<lsum/fb_l.size()<<'\t'<<lp_n/fb_l.size()<<"\t"<<lsum_filter/fb_l.size()<<'\t'<<lp_n_filter/fb_l.size()<<endl;
		cout<<bu_path<<"right:"<<rsum/fb_r.size()<<'\t'<<rp_n/fb_r.size()<<'\t'<<rsum_filter/fb_r.size()<<'\t'<<rp_n_filter/fb_r.size()<<endl<<endl;
    }

};
Test test;

void prepare(string bu_path)
{
  // FILE* f1 = fopen("./entity2id.txt","r");
	// FILE* f2 = fopen("./relation2id.txt","r");
  FILE* f1 = fopen((bu_path+"./entity2id.txt").c_str(),"r");
	FILE* f2 = fopen((bu_path+"./relation2id.txt").c_str(),"r");
	int x;
    ...
    FILE* f_kb = fopen("../data/freebase_ids_label_without_punctuation_without_duplicates_test_top200kEntities","r");
    ...
    fclose(f_kb);
    FILE* f_kb1 = fopen("../data/freebase_ids_label_without_punctuation_without_duplicates_train_top200kEntities","r");
	while (fscanf(f_kb1,"%s",buf)==1)
    {
        ...
        entity2num[relation2id[s2]][entity2id[s1]]+=1;
        entity2num[relation2id[s2]][entity2id[s3]]+=1;
        e2num[entity2id[s1]]+=1;
        e2num[entity2id[s3]]+=1;
        test.add(entity2id[s1],entity2id[s3],relation2id[s2],false);
    }
    fclose(f_kb1);
    FILE* f_kb2 = fopen("../data/freebase_ids_label_without_punctuation_without_duplicates_valid_top200kEntities","r");
...
{
    if (argc<3)
        // argv[1]==bern argv[2]==backup folder
        return 0;
    else
    {
        version = argv[1];
