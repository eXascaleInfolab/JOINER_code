...
// gcc compute-accuracy_euclidean.c -o compute-accuracy_euclidean -lm
// ./compute-accuracy_euclidean emb_text.bin < ../questions-words_lower.txt
// ./compute-accuracy_euclidean backup/0/emb_text.bin < ../data/questions-phrases_lower.txt
...
int main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    if ((!strcmp(st1, ":")) || (!strcmp(st1, "EXIT")) || feof(stdin)) {
      if (TCN == 0) TCN = 1;
      // if (QID != 0) {
      //   printf("%s ACCURACY TOP1: %.2f %%  (%d / %d)\n", argv[1], CCN / (float)TCN * 100, CCN, TCN);
      //   printf("%s Total accuracy: %.2f %%   Semantic accuracy: %.2f %%   Syntactic accuracy: %.2f %% \n", argv[1], CACN / (float)TACN * 100, SEAC / (float)SECN * 100, SYAC / (float)SYCN * 100);
      // }
      QID++;
      scanf("%s", st1);
      if (feof(stdin)) break;
    ...
    for (c = 0; c < words; c++) {
      ...
      if (c == b3) continue;
      dist = 0;
      for (a = 0; a < size; a++) dist += (vec[a] - M[a + c * size])* (vec[a] - M[a + c * size]); //euclidean
      for (a = 0; a < N; a++) {
        if (dist < bestd[a]) { //euclidean
          for (d = N - 1; d > a; d--) {
            bestd[d] = bestd[d - 1];
